...
def get_data_Estonia():
    EE=pd.read_csv('Estoniacases.csv')
    positive=EE['Confirmed cases'].diff().fillna(EE['Confirmed cases'])
    deaths=EE.Deaths.diff().fillna(EE.Deaths)
    date=pd.to_datetime(EE.DateTime)

    EE=pd.DataFrame({'date':date,'positive':positive,'deaths':deaths,'total':None,'state':'Estonia','datasource':'koroonakaart.ee'})
...
def get_data_Luxemburg(plot=False):
    url='https://data.public.lu/fr/datasets/r/615dc0ae-535a-4add-a51d-0722d1383198'
    ...
    # tested=LU['Personnes testées (résidents)']
    deaths=(pd.to_numeric(LU['[1.NbMorts]'].str.extract('(\d+)',
        expand=False)).fillna(0)).diff().fillna(pd.to_numeric(LU['[1.NbMorts]'].str.extract('(\d+)',
        expand=False)).fillna(0))
...
def get_data_Switzerland(plot=False):
    ...
    #url='https://www.covid19.admin.ch/api/data/20201113-z5poj70l/downloads/sources-csv.zip'
    


    base_url='https://www.covid19.admin.ch/en/overview'
    sourcePage = urllib.request.urlopen(base_url)
    soup = BeautifulSoup(sourcePage.read(),features="lxml")
    links = soup.find_all('a')

    for link in links:
        href = link['href']
        if 'csv.zip' in href:        
            urlzip=base_url.strip('/en/overview')+href
    
    url = urllib.request.urlopen(urlzip)

    with ZipFile(BytesIO(url.read())) as my_zip_file:
            for contained_file in my_zip_file.namelist():
                if contained_file=='data/COVID19Cases_geoRegion.csv':
                    CH=pd.read_csv(my_zip_file.open(contained_file),sep=',',
                                   usecols=['geoRegion','datum','sumTotal'],parse_dates=['datum'])
                    CH=CH[CH.geoRegion=='CH']
                    CH['positive']=CH.sumTotal.diff().fillna(CH.sumTotal)
                    CH.drop(columns=['geoRegion','sumTotal'],inplace=True)
                    CH.rename(columns={'datum':'date'},inplace=True)
                
                
                if contained_file=='data/COVID19Death_geoRegion.csv':
                    CH2=pd.read_csv(my_zip_file.open(contained_file),sep=',',
                                   usecols=['geoRegion','datum','sumTotal'],parse_dates=['datum'])
                    CH2=CH2[CH2.geoRegion=='CH']
                    CH2['deaths']=CH2.sumTotal.diff().fillna(CH2.sumTotal)
                    CH2.drop(columns=['geoRegion','sumTotal'],inplace=True)
                    CH2.rename(columns={'datum':'date'},inplace=True)
                
    CHcases=pd.merge(CH,CH2,on='date',how='outer')
    CHcases.sort_values(by='date',inplace=True)
    ...
    CHtests=pd.read_excel(url,parse_dates=['Datum'])
    CHtests=CHtests.groupby(by='Datum',as_index=False).sum()
    CHtests.rename(columns={'Number_of_tests':'total','Datum':'date'},inplace=True)
    CHtests.sort_values(by='date',inplace=True)


    CH=pd.merge(CHcases,CHtests,on='date',how='outer')


    CH.sort_values(by='date',inplace=True)
    CH.fillna(0,inplace=True)

    CH['state']='Switzerland'
    CH['datasource']='www.covid19.admin.ch'
...
def get_data_SwitzerlandOLD(plot=False):
...
def get_data_Portugal(plot=False):
    ...
    PTdf['state']='Portugal'
    PTdf['hospitalised']=PTdf.internados
    PTdf['ICU']=PTdf.internados_uci


    PTdf = PTdf[['state','date','positive','deaths','hospitalised','ICU']]

    PTdf['datasource']='github.com/dssg-pt/covid19pt-data'

    url='https://github.com/dssg-pt/covid19pt-data/raw/master/amostras.csv'
    PTtests=pd.read_csv(url,parse_dates=['data'],usecols=['data','amostras'],dayfirst=True)
    PTtests['total']=PTtests.amostras.diff().fillna(PTtests.amostras)
    PTtests.drop(columns='amostras',inplace=True)
    PTtests.rename(columns={'data':'date'},inplace=True)

    PTdf=pd.merge(PTdf,PTtests,on='date',how='outer')
...
def get_data_Germany(plot=False):
    ...
    GEcases['state']='Germany'
    GEcases['datasource']='opendata.arcgis.com/datasets/dd4580c810204019a7b8eb3e0b329dd6_0.csv'


    file='Testzahlen-gesamt.xlsx'
    df=pd.read_excel(file,sheet_name='Testzahlen',header=2,skipfooter=2)
    df['week']=pd.to_numeric(df['Kalenderwoche 2020'].astype(str).\
                             apply(lambda x: ''.join(ch for ch in x if ch.isdigit())))
    ...
    def getDateRangeFromWeek(p_week):

        p_year=2020
        p_week=str(int(p_week))
    ...
    df2=pd.DataFrame({'weekending':pd.to_datetime(df.week.apply(getDateRangeFromWeek)),'total':df['Anzahl Testungen']})
    GEtests=resample_weekly2daily(df2,'weekending','date')


    DE=pd.merge(GEcases,GEtests,on='date',how='outer')
...
def get_data_France(plot=False):# France new data

#Download data from https://www.data.gouv.fr/en/datasets/donnees-relatives-aux-resultats-des-tests-virologiques-covid-19
...
# Download data from https://www.data.gouv.fr/en/datasets/donnees-relatives-a-lepidemie-de-covid-19-en-france-vue-densemble
    url='https://www.data.gouv.fr/fr/datasets/r/d3a98a30-893f-47f7-96c5-2f4bcaaa0d71'

    df=pd.read_csv(url,parse_dates=['date'])

    df.date[df.date=='2020_11-11']='2020-11-11'

    df.date[df.date=='2020-11_11']='2020-11-11'
    ...
    df=df.reset_index()
    df['positive']=df.total_cas_confirmes.diff().fillna(df.total_cas_confirmes)
    
    df['deathssum']=(df.total_deces_hopital+df.total_deces_ehpad).fillna(0)
    df['deaths']=df.deathssum.diff().fillna(df.deathssum)
    
    df=pd.DataFrame({'date':df.date,'positive':df.positive,'deaths':df.deaths})
    
    
    FR=pd.merge(fr,df,on='date',how='outer')
    ...
    FR.sort_values(by='date',inplace=True)
...
def get_data_Malta(plot=False):
    ...
    MTurl='https://github.com/COVID19-Malta/COVID19-Cases/raw/master/COVID-19%20Malta%20-%20Aggregate%20Data%20Set.csv'
    df=pd.read_csv(MTurl)
    positive=df['New Cases']
    deaths=df.Deaths.diff().fillna(df.Deaths)
    date=pd.to_datetime(df.Date.astype(str)+'-'+df.Year.astype(str),format="%d-%b-%Y")


    MTdf=pd.DataFrame({'date':date,'positive':positive,'deaths':deaths})

    url='https://raw.githubusercontent.com/COVID19-Malta/COVID19-Cases/master/COVID-19%20Malta%20-%20Number%20of%20PCR%20Tests%20by%20Date.csv'

    tests=pd.read_csv(url,usecols=['Date','Change in cumulative total'])

    date=pd.to_datetime(tests.Date,dayfirst=True)

    total=tests['Change in cumulative total']

    MTtests=pd.DataFrame({'date':date,'total':total})

    MT=pd.merge(MTdf,MTtests,on='date',how='outer')

    MT.sort_values(by='date',inplace=True)
    
    MT['state']='Malta'
    MT['datasource']='covid19dashboard.gov.mt'


    if plot:
        MT.plot(x='date',y=['deaths','total','positive'])   

    
    return MT


def get_data_MaltaOLD(plot=False):
...
def get_data_Slovenia(plot=False):

    SOurl='https://www.gov.si/assets/vlada/Koronavirus-podatki/en/EN_Covid-19-all-data.xlsx'
    SO=pd.read_excel(SOurl,skipfooter=21)
    SO=pd.DataFrame({'date':pd.to_datetime(SO['Date']),\
                                        'positive':SO['Positive (daily)'],\
                                       'deaths':SO['Deaths (daily)'].replace('\*','',regex=True).astype(float),\
                                         'total':SO['Tested (daily)']})

    SO['state']='Slovenia'
    SO['datasource']='www.gov.si/en/topics/coronavirus-disease-covid-19/actual-data'
