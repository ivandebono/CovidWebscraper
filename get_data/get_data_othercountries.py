...
def get_data_Canada(plot=False):
    ...
    CNDdf.rename(columns={'numtoday':'positive','numtestedtoday':'total','numdeathstoday':'deaths'},inplace=True)
...
def get_data_Poland():
    ...
    date=pd.to_datetime(((tests.Data.apply(lambda x: "{:.2f}".format(x))).astype(str))+'.2020',dayfirst=True)
    ...
    date=pd.to_datetime(((cases.Data.apply(lambda x: "{:.2f}".format(x))).astype(str))+'.2020',dayfirst=True)
...
#     SE=pd.read_csv(url,usecols=['Statistikdatum','Totalt_antal_fall','Antal_avlidna'])
#     SE['date']=pd.to_datetime(SE.Statistikdatum)
#     SE.rename(columns={'Totalt_antal_fall':'positive','Antal_avlidna':'deaths'},inplace=True)
#     SE['state']='Sweden'
#     SE['datasource']='www.folkhalsomyndigheten.se'
#     SE['total']=None
#     
#     return SE